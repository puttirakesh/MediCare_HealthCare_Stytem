...
// HELPER FUNCTION TO UPLOAD DOCTOR'S CERTIFICATE TO CLOUDINARY

//convert time string like "9:00 AM" to minutes (e.g., 9:00 AM -> 540)
...
// DEDUPLICATE AND SORT SLOTS IN THE SCHEDULE
...
// PARSE SCHEDULE INPUT FROM CLIENT (CAN BE STRINGIFIED JSON OR OBJECT)
...
// NORMALIZE DOCTOR DOCUMENT FOR CLIENT RESPONSE
...
export async function createDoctor(req, res) {
    try {
        const body = req.body || {};
        if (!body.email || !body.specialization || !body.name) {
...
// to get all doctors with search and pagination
...
// to update a doctor by id (only the doctor themselves can update their profile)
...
// to delete a doctor by id (only the doctor themselves can delete their profile)
...
// to toggle doctor availability (only the doctor themselves can toggle their availability)
