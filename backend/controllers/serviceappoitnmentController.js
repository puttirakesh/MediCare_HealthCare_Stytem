...
import Stirpe from "stripe";
import { getAuth } from "@clerk/express";

const stripekey = process.env.STRIPE_SECRET_KEY || null;
const stripe = stripekey ? new Stirpe(stripekey, { apiVersion: "2022-11-15" }) : null;
...
export const confirmServicePayment = async (req, res) => {
    try {
        ...
        try {
            ...
            return res.status(404).json({ success: false, message: "Stripe not found !" });
        }
...
export const getServiceAppointments = async (req, res) => {
    try {
        ...
        const appointments = await ServiceAppointment.find(filter)
            .populate("serviceId", "name image imageUrl imageSmall").sort({ createdAt: -1 }).skip(skip).limit(limit).lean();
...
export const updateServiceAppointment = async (req, res) => {
    try {
        ...
        const updated = await ServiceAppointment.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
...
// to get service appointments by patient
export const getServiceAppointmentsByPatient = async (req, res) => {
    try {
        ...
        if (!resolvedCreatedBy && !mobile) return res.status(400).json({ success: false, data: [] });
