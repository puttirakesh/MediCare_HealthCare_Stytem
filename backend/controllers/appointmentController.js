...
// console.log("STRIPE INIT:", !!stripe);
//helper funstions
const safeNumber = (v) => {
    const n = Number(v);
...
export const getAppointments = async (req, res) => {
    try {
        ...
        const items = await Appointment.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit)
            .populate("doctorId", "name specialization owner image url").lean();
...
// to get appointment by patients
export const getAppointmentByPatient = async (req, res) => {
    try {
        ...
        if (!resolvedCreatedBy && !req.query.mobile) {
            ...
            return res.status(400).json({
                ...
                debug: {
                    ...
                    hasQueryCreatedBy: !!req.query.createdBy,
                    hasMobile: !!req.query.mobile
                }
            });
        }
        ...
        if (resolvedCreatedBy) {
            ...
            filter.$or = [
                { patientId: resolvedCreatedBy },
                { createdBy: resolvedCreatedBy },
                { userId: resolvedCreatedBy },
                { clerkUserId: resolvedCreatedBy }
            ];
        }
...
export const createAppointment = async (req, res) => {
    try {
        ...
        if (!clerkUserId) return res.status(401).json({ success: false, message: "Authentication is required to access this resource" });
        ...
        if (numericFee === null || numericFee < 0) {
            return res.status(400).json({ success: false, message: "Please provide valid fee or fees" });
        }
        ...
        if (existingBooking) {
            return res.status(400).json({ success: false, message: "Duplicate booking not allowed" });
        }
        let doctor = null;
        try {
...
export async function confirmPayment(req, res) {
    try {
        ...
        if (session.payment_status !== "paid") {
            return res.status(400).json({ success: false, message: "Payment is not paid yet" });
        }
...
export const createCheckoutSession = async (req, res) => {
    try {
        const { amount, doctorId, patientName, mobile } = req.body;
...
export async function getAppointmentsByDoctor(req, res) {
    try {
        ...
        const items = await Appointment.find(filter).sort({date:1, time:1}).skip(skip).limit(limit).populate("doctorId", "name specilization owner imageUrl image").lean();

        const total = await Appointment.countDocuments(filter);
        return res.json({ success: true, appointments: items, meta: {page, limit, total, count: items.length }});

    } catch (error) {
        console.error("getAppointments BY Doctor Error: ", error);
        return res.status(500).json({ success: false, error: "Internal Server Error" });
    }
