...
import { Calendar, CheckCircle, Dice1, Eye, EyeClosed, Plus, Trash2, User, XCircle } from 'lucide-react';
...
const AddPage = () => {
        ...
        return () => {
            ...
            if (form.imagePreview && form.imageFile) {
                try {
                    ...
                    console.warn("Error cleaning up image URL:", err);
                }
            }
        };
    }, []);
    ...
    function addSlotToForm() {
        ...
        const existingSlots = form.schedule[slotDate] || [];
        ...
        setSlotHour("");
        setSlotMinute("00");
        // Don't clear date to allow adding multiple slots on same day
    }
    ...
    function validate(f) {
        ...
        for (let k of req) {
            if (!f[k] || f[k].toString().trim() === "") {
                showToast("error", `${k.charAt(0).toUpperCase() + k.slice(1)} is required`);
                return false;
            }
        }

        if (!f.imageFile) {
            showToast("error", "Please upload a profile image");
            return false;
        }

        if (!Object.keys(f.schedule).length) {
            showToast("error", "Please add at least one time slot");
            return false;
        }
    ...
    async function handleAdd(e) {
        ...
        try {
            ...
            const doctorFromServer = data?.data ? {
                ...
                imageUrl: data.data.imageUrl || form.imagePreview,
        ...
        <div className={s.pageContainer}>
                ...
                <div className={s.maxWidthContainer + " " + s.formContainer}>
                    <form onSubmit={handleAdd} className={s.formGrid}>
                            ...
                            <div className='flex flex-wrap items-center gap-4'>
                                ...
                                {form.imagePreview && (
                                    <div className='relative-group'>
                                        <img src={form.imagePreview} className={s.imagePreview} alt="preview" />
                        ...
                        <textarea className={s.textareaBase + " md:col-span-2"}
                            rows={3} placeholder='About Doctor' value={form.about} onChange={(e) => setForm({ ...form, about: e.target.value, })}>
                        </textarea>
                        ...
                        <div className={s.submitButtonContainer}>
                            ...
                            >
                                {loading ? "Adding Doctor..." : "Add Doctor to Team"}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div className={s.doctorListContainer}>
                {doctorList.length ? (
                    ...
                    <p className={s.emptyState}>No Doctor</p>
                )}
            </div>
        </div>
    )
