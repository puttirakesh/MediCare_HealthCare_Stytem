...
import { addServiceStyles as as } from '../assets/dummyStyles.js'
...
const AddservicesPage = ({ serviceId }) => {
    ...
    // to fetch the service details from the server when in editing state
    useEffect(() => {
        let mounted = true;
    ...
    return (
        <div className={as.container.main}>
            <div className={as.toast.container}>
                {toast && (
                    <div className={`${as.toast.toastBase} ${toast.type === "error" ? as.toast.toastError :
                        toast.type === "info" ? as.toast.toastInfo :
                            as.toast.toastSuccess
                        }animate-slideIn`}>
                        <div className={as.toast.iconContainer(toast.type)}>
                            {toast.type === "error" ? (
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className={as.toast.message}>
                                {toast.message}
                            </div>
                        </div>

                        <button onClick={() => setToast(null)}
                            className={as.buttons.toastClose}>
                            <XCircle className="w-5 h-5 text-grey-400 hover:text-grey-600" />
                        </button>
                    </div>
                )}
            </div>

            <form onSubmit={handleSubmit} className={as.container.form}>
                <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4'>
                    ...
                    <div className={as.headerActions}>
                        <button type='button' onClick={resetForm} className={as.buttons.reset}>
                            Reset
                        </button>
                        <button type='submit' disabled={submitting} className={as.buttons.submit}>
                            {submitting ? (
                                <>
                ...
                <div className={as.grids.main}>
                    <div className=' lg:col-span-1 md:col-span-1 col-span-1 flex flex-col items-center'>
                        <div className={as.imageUpload.container(errors.image)}>
                            ...
                            <div
                                ...
                                className={`${as.imageUpload.preview} cursor-pointer`}
                            >
                                {imagePreview ? (
                                    <img
                                        src={imagePreview}
                                    ...
                                    <div className={as.imageUpload.placeholder}>
                            ...
                            <div className=" w-full flex gap-2 items-center">
                                ...
                                <button type='button' onClick={() => fileRef.current?.click()}
                                    className={as.buttons.uploadImage}>
                                ...
                                {(imagePreview || hasExistingImage) && (
                                    <button
                                        ...
                                        className={as.buttons.removeImage}
                                    >
                    ...
                    <div className="lg:col-span-2 md:col-span-1 col-span-1 space-y-6">
                        <div className={as.grids.formFields}>
                            <div>
                                <label className={as.labels.standard}>
                                    Service name
                                </label>
                                <input
                                    value={serviceName}
                                    ...
                                    className={as.formFields.input(errors.serviceName)}
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="₹ 499"
                                    className={as.formFields.input(errors.price)}
                                    inputMode="numeric"
                                />
                                ...
                                <div className="mt-3">
                                    ...
                                    <select
                                        ...
                                        className={as.formFields.select}
                                    >
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={4}
                                className={as.formFields.textarea(errors.about)}
                            />
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className={as.buttons.addInstruction}
                                >
                            ...
                            <div
                                className={as.instructions.container(errors.instructions)}
                            >
                                {instructions.map((ins, idx) => (
                                    <div
                                        key={idx}
                                        className={as.instructions.item}
                                    >
                                        <div className={as.icon.number}>
                                            {idx + 1}.
                                        </div>
                                        <input
                                            value={ins}
                                            ...
                                            className={as.instructions.input}
                                        />
                                        {instructions.length > 1 && (
                                            <button
                                                type="button"
                                                onClick={() => removeInstruction(idx)}
                                                className={as.instructions.removeButton}
                                            >
                                                <Trash2 className={as.icon.removeInstruction} />
                                            </button>
                                        )}
                                    </div>
                                ))}
                        ...
                        {/* slot controls */}
                        <div
                            className={as.slots.container(errors.slots)}
                        >
                            ...
                            <div className={as.grids.timeGrid}>
                                <div className="min-w-0">
                                    <label className={as.labels.small}>Day</label>
                                    <select
                                        value={slotDay}
                                        onChange={(e) => setSlotDay(e.target.value)}
                                        className={as.formFields.smallSelect}
                                    >
                                        {days.map((d) => {
                                            const dNum = Number(d);
                                            const disabled =
                                ...
                                <div className="min-w-0">
                                    <label className={as.labels.small}>Month</label>
                                    <select
                                        value={slotMonth}
                                ...
                                <div className="min-w-0">
                                    <label className={as.labels.small}>Year</label>
                                    <select
                                        value={slotYear}
                                ...
                                <div className={as.grids.timeSubGrid}>
                                    <div className="min-w-0">
                                        <label className={as.labels.small}>Hour</label>
                                        <select
                                            value={slotHour}
                                            onChange={(e) => setSlotHour(e.target.value)}
                                            className={as.formFields.timeSelect}
                                        >
                                            {hours.map((h) => (
                                    ...
                                    <div className="min-w-0">
                                        <label className={as.labels.small}>Minute</label>
                                        <select
                                            value={slotMinute}
                                    ...
                                    <div className="min-w-0">
                                        <label className={as.labels.small}>AM/PM</label>
                                        <select
                                            value={slotAmPm}
                                            onChange={(e) => setSlotAmPm(e.target.value)}
                                            className={as.formFields.ampmSelect}
                                        >
                                            {ampm.map((a) => (
                            ...
                            <div>
                                ...
                                <div className={as.grids.slotsGrid}>
                                    {slots.length === 0 ? (
                                        ...
                                        slots.map((s, idx) => (
                                            <div
                                                key={s}
                                                className={as.slots.slotItem}
                                            >
                                                <div className="flex items-center gap-2 min-w-0">
                                                    <Clock className={as.icon.clock} />
                                                    <div className={as.slots.slotText}>
                                                        {s}
                                                    </div>
                                                </div>
                                                <button
                                                    ...
                                                    className={as.buttons.slotRemove}
                                                >
                                                    <Trash2 className={as.icon.trash} />
                                                </button>
                                            </div>
                                        ))
                                    )}
