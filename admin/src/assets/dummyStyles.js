...
export const doctorListStyles = {
  // ===== LAYOUT =====
container: "min-h-screen bg-gradient-to-br from-emerald-50 to-white font-inter p-6",
  ...
  headerIcon:
    "p-2 rounded-full bg-white shadow-sm border border-gray-100",
  ...
  headerSearchContainer:
    "w-full sm:w-auto flex flex-col sm:flex-row gap-3",

  searchBox:
    "flex items-center w-full sm:w-80 bg-white rounded-full px-4 py-2 border border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-emerald-400",

  searchIcon: "text-gray-400",
  searchInput:
    "ml-2 w-full outline-none text-gray-700 placeholder-gray-400 bg-transparent",

  clearButton:
    "px-4 py-2 rounded-full bg-emerald-600 text-white text-sm shadow hover:bg-emerald-700 transition",

  // ===== FILTER =====
  filterContainer: "flex flex-wrap gap-2 pt-4",
  ...
  filterButton: (isActive, color) => {
    ...
    if (color === "red") {
      return isActive
        ...
        : `${base} bg-white text-red-600 border-red-200 hover:bg-red-50`;
    }
  ...
  // ===== STATES =====
  loadingContainer: "text-center text-gray-500 py-8",
  noResultsContainer: "text-center text-gray-500 py-8",

  // ===== GRID =====
  gridContainer:
    "max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5",

  // ===== CARD =====
  article:
  "bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition",
  articleContent:
  ...
  // ===== INFO =====
  doctorInfoContainer: "flex-1 w-full",
  ...
  // ===== RATING =====
  ratingContainer: "flex items-center gap-3 mt-2",
  rating:
    "flex items-center gap-1 text-sm text-yellow-500 font-medium",

  toggleButton: (isOpen) =>
    `p-2 rounded-full bg-gray-50 hover:bg-gray-100 transition ${
      isOpen ? "rotate-180" : ""
    }`,

  // ===== STATS =====
  statsContainer:
    "flex flex-col items-start md:items-end gap-2 text-sm",

  statsLabel: "text-xs text-gray-400",
  statsValue:
    "flex items-center gap-1 text-gray-700 font-medium",
  ...
  deleteButton:
    "text-xs px-3 py-1 bg-red-50 text-red-600 rounded-full hover:bg-red-100 flex items-center gap-1 transition",

  feesLabel:
    "text-sm font-medium text-gray-600",

  feesValue:
    "flex items-center gap-1 text-emerald-700 font-semibold",

  // ===== EXPAND SECTION =====
  expandableContent: "",

  expandCard:
    "grid grid-cols-1 md:grid-cols-3 gap-6 mt-4 p-5 bg-gray-50 rounded-xl border border-gray-100",
  ...
  scheduleSlot:
    "text-xs px-2 py-0.5 bg-white border rounded-md text-gray-600 shadow-sm",
  ...
  // ===== RIGHT SIDEBAR =====
  statsSidebar:
    "flex md:flex-col justify-between md:justify-start gap-4 text-sm text-right",
  ...
  // ===== SHOW MORE =====
  showMoreButton:
    "px-5 py-2 rounded-full bg-white border border-gray-200 shadow-sm hover:shadow-md transition text-sm",
...
export const heroStyles = {
  // Container styles
  container: "min-h-screen font-sans bg-gradient-to-br from-emerald-50 via-white to-teal-50",
  mainContainer: "flex items-center pt-28 justify-center px-6 py-16",
  section: "w-full max-w-6xl",

  // Background decoration styles
  decorativeBg: {
    container: "relative",
    animatedGradient: "absolute inset-0 -z-20 opacity-30 bg-gradient-to-r from-emerald-200 via-teal-200 to-emerald-100 blur-3xl animate-pulse",
    blurBackground: "absolute -inset-8 -z-10 flex items-center justify-center",
    blurShape: "w-full h-56 md:h-72 rounded-full bg-emerald-200/40 blur-3xl",
    floatingElements: "absolute inset-0 -z-5 overflow-hidden pointer-events-none"
  },

  // Content container
  contentBox: "bg-white/80 backdrop-blur-xl border border-white/40 rounded-3xl shadow-2xl p-8 md:p-12 text-center transition-all duration-300 hover:shadow-emerald-100/30",

  // Logo/Image styles with animation
  logoContainer: "mx-auto mb-6 w-28 h-28 flex items-center justify-center rounded-full bg-gradient-to-br from-emerald-100 to-teal-100 shadow-lg p-2",
  logo: "w-20 h-20 object-contain transition-transform duration-300 hover:scale-110",

  // Text styles with gradient
  heading: "text-4xl md:text-5xl lg:text-6xl font-extrabold bg-gradient-to-r from-emerald-700 via-teal-700 to-emerald-800 bg-clip-text text-transparent mb-3",
  description: "text-gray-600 max-w-2xl mx-auto leading-relaxed mb-8 text-lg",

  // Welcome badge
  welcomeBadge: "inline-flex items-center gap-2 bg-emerald-100/80 backdrop-blur-sm px-4 py-2 rounded-full text-emerald-700 text-sm font-semibold mb-4 border border-emerald-200",

  // Info cards styles - enhanced
  infoCards: {
    container: "mx-auto max-w-5xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8",
    card: "group p-6 rounded-2xl bg-white/90 backdrop-blur-sm border border-emerald-100/50 text-left transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-emerald-200 cursor-pointer",
    cardTitle: "font-bold text-emerald-800 text-lg mb-2 group-hover:text-emerald-600 transition-colors",
    cardText: "text-sm text-gray-600 leading-relaxed",
    iconWrapper: "w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-all duration-300 group-hover:scale-110",
    iconEmerald: "bg-emerald-100 text-emerald-600",
    iconBlue: "bg-blue-100 text-blue-600",
    iconPurple: "bg-purple-100 text-purple-600",
    iconOrange: "bg-orange-100 text-orange-600",
    iconRose: "bg-rose-100 text-rose-600"
  },

  // Stats section
  statsContainer: "grid grid-cols-2 md:grid-cols-4 gap-4 mb-8",
  statCard: "bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-emerald-100/50 text-center transition-all duration-300 hover:bg-white/80",
  statValue: "text-2xl md:text-3xl font-bold text-emerald-700",
  statLabel: "text-xs text-gray-500 uppercase tracking-wider mt-1",

  // CTA Button
  ctaContainer: "mt-8 flex flex-col sm:flex-row gap-4 justify-center items-center",
  ctaButton: "px-8 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-xl hover:from-emerald-700 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5",
  secondaryButton: "px-8 py-3 bg-white/80 backdrop-blur-sm text-emerald-700 font-semibold rounded-xl border border-emerald-200 hover:bg-emerald-50 transition-all duration-300",

  // Quick actions section
  quickActions: {
    container: "mt-12 pt-8 border-t border-emerald-100",
    title: "text-xl font-bold text-emerald-800 mb-6 text-center",
    grid: "grid grid-cols-2 md:grid-cols-4 gap-3",
    action: "flex items-center justify-center gap-2 px-4 py-3 bg-white/60 rounded-xl border border-emerald-100 hover:bg-emerald-50 transition-all duration-200 cursor-pointer text-sm font-medium text-gray-700 hover:text-emerald-700"
  },

  // Notification bell
  notification: {
    container: "fixed top-24 right-6 z-50",
    button: "relative bg-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer border border-emerald-100",
    badge: "absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold"
  }
};
...
export const serviceAppointmentsStyles = {
  container: "p-4 sm:p-6 md:p-6 font-sans bg-gray-50 min-h-screen", // Changed from font-serif to font-sans

  headerContainer: "flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6",
  headerTitleContainer: "min-w-0",
  headerTitle: "text-2xl md:text-3xl font-bold text-emerald-800 tracking-tight", // Improved typography
  headerSubtitle: "text-sm text-gray-500 mt-1",

  searchContainer: "w-full md:w-auto flex flex-col gap-2",
  searchInputWrapper: "flex flex-col sm:flex-row gap-3",
  searchLabel: "relative block flex-1",
  searchIconContainer: "absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none",
  searchIcon: "w-4 h-4 text-emerald-400",
  searchInput: "pl-10 pr-10 w-full rounded-full border border-emerald-200 bg-white py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-300 transition-all",
  clearSearchButton: "absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 hover:bg-gray-100 transition-colors",
  clearSearchIcon: "w-4 h-4 text-gray-500",

  statusFilterSelect: "text-sm px-4 py-2.5 cursor-pointer rounded-full border border-emerald-300 bg-white hover:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-200",
  searchInfo: "mt-2 text-xs text-gray-500 flex items-center justify-between",
  refreshButton: "text-xs text-emerald-600 hover:text-emerald-700 font-medium transition-colors",

  loadingContainer: "col-span-full rounded-2xl p-8 bg-white border border-emerald-50 shadow-sm flex items-center justify-center gap-3 text-emerald-600",
  errorContainer: "col-span-full rounded-2xl p-4 bg-rose-50 border border-rose-100 text-rose-700",

  noResultsContainer: "col-span-full rounded-2xl p-12 bg-white border border-emerald-50 shadow-sm flex items-center justify-center flex-col gap-3",
  noResultsIcon: "text-4xl text-emerald-300 mb-4",
  noResultsText: "text-base text-gray-600 font-medium",
  noResultsSubtext: "text-sm text-gray-400",

  gridContainer: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6",

  article: "group relative rounded-2xl transform transition-all duration-300 hover:-translate-y-1 hover:shadow-xl",
  cardInner: "rounded-2xl overflow-hidden border border-emerald-200 bg-white shadow-md h-full flex flex-col transition-all duration-200 hover:border-emerald-300",

  cardHeader: "flex flex-col sm:flex-row sm:items-start justify-between gap-3 p-5 pb-3",
  patientInfoContainer: "flex items-start gap-3 min-w-0 flex-1",
  patientAvatar: "rounded-full w-10 h-10 flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-emerald-100 to-emerald-50",
  patientAvatarIcon: "h-5 w-5 text-emerald-600",
  patientInfo: "min-w-0 flex-1",
  patientName: "text-base font-semibold text-gray-900 truncate", // Fixed truncation
  patientDetails: "text-xs text-gray-500 mt-0.5",

  statusContainer: "flex flex-col items-start sm:items-end gap-2",
  statusSelect: (terminal) =>
    `text-xs cursor-pointer px-2.5 py-1 rounded-full border focus:outline-none transition-all ${terminal
      ? "bg-gray-50 text-gray-400 cursor-not-allowed border-gray-200"
      : "bg-white text-emerald-700 border-emerald-300 hover:border-emerald-400"
    }`,

  detailsContainer: "px-5 py-2 flex flex-col gap-2.5 text-gray-700",
  detailItem: "flex items-center gap-2.5 text-sm",
  detailIcon: "w-4 h-4 text-emerald-500 flex-shrink-0",
  detailText: "text-gray-700 truncate",
  feesText: "font-semibold text-emerald-700",
  serviceText: "mt-1 text-sm text-gray-600 line-clamp-2", // Added line-clamp for multi-line
  serviceName: "font-medium text-emerald-700",

  actionsContainer: "mt-auto pt-3 px-5 pb-5",
  actionsInnerContainer: "flex items-center gap-2",
  cancelButton: (isLocked) =>
    `px-3 py-1.5 rounded-full text-sm font-medium transition-all ${isLocked
      ? "bg-gray-100 text-gray-400 cursor-not-allowed"
      : "bg-rose-50 text-rose-600 hover:bg-rose-100 border border-rose-200"
    }`,

  legendContainer: "mt-8 p-4 rounded-xl bg-white border border-emerald-100 shadow-sm text-sm flex flex-wrap items-center justify-center gap-6",
  legendItem: "flex items-center gap-2",
  legendDot: "w-4 h-4 rounded-full shadow-sm flex-shrink-0",
  // StatusBadge styles
  statusBadge: (status) => {
    const map = {
    ...
    const classes = map[status] || "bg-gray-50 text-gray-700 border border-gray-200";
    return `inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${classes}`;
  },

  // RescheduleButton styles
  rescheduleButton: (terminal) =>
    `w-full text-sm px-3 py-1.5 rounded-full font-medium transition-all ${terminal
      ? "bg-gray-100 text-gray-400 cursor-not-allowed"
      : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200"
    }`,
  rescheduleEditContainer: "flex flex-col gap-2 bg-gray-50 p-3 rounded-lg",
  rescheduleDateInput: "text-sm px-3 py-2 w-full border border-emerald-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-200",
  rescheduleTimeInput: "text-sm px-3 py-2 w-full border border-emerald-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-200",
  rescheduleActions: "flex gap-2",
  rescheduleSaveButton: "flex-1 px-3 py-2 bg-emerald-500 text-white rounded-lg text-sm font-medium hover:bg-emerald-600 transition-colors",
  rescheduleCancelButton: "flex-1 px-3 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300 transition-colors",

  // Toast styles
  toastContainer: "fixed top-4 right-4 z-50 flex flex-col gap-2",
  toast: "w-80 rounded-lg shadow-lg bg-white border-l-4 border-emerald-400",
  toastContent: "flex items-start gap-3 p-4",
  toastSpinner: "h-5 w-5 animate-spin text-emerald-500",
  toastText: "flex-1",
  toastTitle: "font-semibold text-sm text-gray-900",
  toastMessage: "text-xs text-gray-600 mt-0.5",
  toastCloseButton: "text-gray-400 hover:text-gray-600 transition-colors",
...
export const serviceDashboardStyles = {
  ...
  table: {
    ...
    // serviceDashboardStyles.js
    desktopCell2: "col-span-2",
    desktopCell1: "col-span-1 text-center",
    desktopCell5: "col-span-5",
