...
import { listPageStyles as l } from '../assets/dummyStyles.js'
...
function StatusBadge({ status }) {
  const base = l.statusBadgeBase;
  if (status === "complete")
    return (
      <span className={`${base} ${l.statusBadgeComplete}`}>
        Completed
      </span>
    );
  if (status === "cancelled")
    return (
      <span className={`${base} ${l.statusBadgeCancelled}`}>
        Cancelled
      </span>
    );
  if (status === "confirmed")
    return (
      <span className={`${base} ${l.statusBadgeConfirmed}`}>
        Confirmed
      </span>
    );
  if (status === "rescheduled")
    return (
      <span className={`${base} ${l.statusBadgeRescheduled}`}>
        Rescheduled
      </span>
    );
  return (
    <span className={`${base} ${l.statusBadgePending}`}>
      Pending
    </span>
  );
}
  ...
  if (appointment.status === "rescheduled") {
    return (
      <select
        ...
        className={`${l.statusSelect} ${terminal
          ? l.statusSelectDisabled
          : l.statusSelectEnabled
          }`}
      ...
      {!editing ? (
        <div className="flex justify-end">
          <button
            ...
            className={`${l.rescheduleButton} ${terminal
              ? l.rescheduleButtonDisabled
              : l.rescheduleButtonEnabled
              }`}
          >
            Reschedule
          </button>
        ...
        <div className={l.rescheduleForm}>
          <input
            type="date"
            value={date}
            min={minDate}
          ...
          <input
            ...
            className={l.timeInput}
          />
          <div className={l.rescheduleButtons}>
            <button onClick={save} className={l.saveButton}>
              Save
            </button>
            <button onClick={cancel} className={l.cancelButton}>
              Cancel
            </button>
          </div>
        </div>
    ...
    <div className={l.pageContainer}>
      <div className={l.contentWrapper}>
        <div className={l.headerContainer}>
          <div>
            <h1 className={l.headerTitle}>All Appoinments</h1>
            <p className={l.headerSubtitle}>Latest at top - search by patient name or date</p>
          </div>
        ...
        {loading ? (
          <div className={l.loadingContainer}>Loading Appointments</div>
        ) : error ? (
              ...
              <article key={a.id} className={l.appointmentCard}>
                <header className={l.cardHeader}>
                  <div className={l.cardAvatar}>
                    {a.doctorImage ? (
                      <img src={a.doctorImage} alt={a.doctorName}
                        className={l.cardAvatarImage} onError={(e) => (e.currentTarget.style.display = "none")} />
                    ) : (
                  ...
                  <div className={l.cardContent}>
                    <div className={l.cardPatientName}>{a.patient}</div>
                    <div className={l.cardPatientInfo}>{a.age} yrs &middot; {a.gender}</div>
                    <div className={l.cardDoctorInfo}>
                      <span className={l.cardDoctorName}>{a.doctorName}</span>
                    </div>
                    <div className={l.cardSpeciality}>{a.speciality}</div>
                  </div>
                </header>
