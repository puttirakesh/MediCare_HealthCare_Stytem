...
import { appointmentPageStyles as a, cardStyles as c, badgeStyles as b, iconSize } from '../assets/dummyStyles.js'
import axios from 'axios';
import { useAuth, useUser } from '@clerk/clerk-react';
import { Toaster } from 'react-hot-toast';
import {
    CalendarDays, Clock, CreditCard, Wallet,
    CheckCircle, Bell, XCircle
} from 'lucide-react';
...
const PaymentBadge = ({ payment }) => {
    return payment === "Online" ? (
        <span className={b.paymentBadge.online}>
            <CreditCard className={iconSize.small} /> Online
        </span>
    ) : (
        <span className={b.paymentBadge.cash}>
...
const StatusBadge = ({ itemStatus }) => {
    if (itemStatus === "Completed")
        return (
            <span className={b.statusBadge.completed}>
    ...
    if (itemStatus === "Confirmed")
        return (
            <span className={b.statusBadge.confirmed}>
    ...
    if (itemStatus === "Pending")
        return (
            <span className={b.statusBadge.pending}>
    ...
    if (itemStatus === "Canceled")
        return (
            <span className={b.statusBadge.canceled}>
                <XCircle className={iconSize.small} /> Canceled
            </span>
        );

    return (
        <span className={b.statusBadge.default}>
    ...
    return (
        <div className={a.pageContainer}>
            ...
            <div className={a.maxWidthContainer}>
                <h1 className={a.doctorTitle}>Your Doctor Appointments</h1>
                {loadingDoctors && (
                    <div className={a.loadingText}>Loading your doctor appointments...</div>
                )}
                ...
                <div className={a.doctorGrid}>
                    {appointmentData && appointmentData.length > 0 ? (
                        appointmentData.map((item, index) => (
                            <div key={item._id || item.id || index} className={`${c.doctorCard} hover:ring-2 hover:ring-emerald-200`}>
                                <div className={c.doctorImageContainer}>
                                    <img
                                        src={item.image || "/placeholder-doctor.png"}
                                        alt={item.doctor || "Doctor"}
                                        className={`${c.image} bg-gray-100`}
                                        loading='lazy'
                                        onError={(e) => {
                                ...
                                <h2 className={c.doctorName}>{item.doctor || "Doctor Name"}</h2>
                                ...
                                {item.status === "Rescheduled" && item.rescheduledTo ? (
                                    <div className={c.rescheduledText}>Reschedule to{" "}
                ...
                <h1 className={a.serviceTitle}>Your Booked Services</h1>
                {loadingServices && (
                    <div className={a.serviceLoadingText}>Loading Service Bookings...</div>
                )}

                {!loadingServices && serviceData.length === 0 && (
                    <div className={a.serviceEmptyStateText}>No service bookings found.</div>
                )}
