export const appointmentPageStyles = {
  pageContainer:
    "min-h-screen mt-20 font-sans bg-gradient-to-br from-slate-50 via-emerald-50 to-teal-100 py-12 px-4",

  maxWidthContainer: "max-w-7xl mx-auto",

  doctorTitle:
    "text-4xl font-bold text-center bg-gradient-to-r from-emerald-600 to-teal-500 bg-clip-text text-transparent mb-10",

  serviceTitle:
    "text-4xl font-bold text-center bg-gradient-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent mb-10",

  loadingText: "text-center text-gray-500 py-6 animate-pulse",
  serviceLoadingText: "text-center text-gray-500 py-6 animate-pulse",

  emptyStateText:
    "text-center text-gray-400 py-10 text-lg italic",
  serviceEmptyStateText:
    "text-center text-gray-400 py-10 text-lg italic",

  doctorGrid:
    "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16",
...
export const cardStyles = {
  doctorCard:
    "group relative bg-white/70 backdrop-blur-lg border border-white/40 rounded-3xl p-6 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col items-center",

  serviceCard:
    "group bg-white/70 backdrop-blur-lg border border-white/40 rounded-3xl p-6 shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300",

  doctorImageContainer:
    "w-28 h-28 rounded-full border-4 border-white shadow-lg bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center overflow-hidden",

  serviceImageContainer:
    "w-28 h-28 rounded-full border-4 border-white shadow-lg bg-gradient-to-br from-blue-100 to-indigo-100 mx-auto flex items-center justify-center overflow-hidden",

  image: "w-full h-full object-cover group-hover:scale-110 transition duration-300",

  doctorName:
    "text-lg font-semibold mt-4 text-center text-gray-800",

  serviceName:
    "text-lg font-semibold text-center mt-4 text-gray-800",

  specialization:
    "text-sm text-gray-500 mt-1 text-center",

  price:
    "text-center text-emerald-600 font-bold text-xl mt-2",

  dateContainer:
    "mt-4 text-sm rounded-full bg-emerald-50 text-emerald-700 px-4 py-1 flex justify-center gap-2",

  serviceDateContainer:
    "mt-4 text-sm rounded-full bg-blue-50 text-blue-700 px-4 py-1 flex justify-center gap-2",

  timeContainer:
    "mt-2 text-sm rounded-full bg-emerald-50 text-emerald-700 px-4 py-1 flex justify-center gap-2",

  serviceTimeContainer:
    "mt-2 text-sm rounded-full bg-blue-50 text-blue-700 px-4 py-1 flex justify-center gap-2",
...
export const badgeStyles = {
  paymentBadge: {
    online:
      "px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700 flex items-center gap-1",
    cash:
      "px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 flex items-center gap-1",
  },

  statusBadge: {
    completed:
      "px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 flex items-center gap-1",

    confirmed:
      "px-3 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700 flex items-center gap-1",

    pending:
      "px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 flex items-center gap-1",

    canceled:
      "px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700 flex items-center gap-1",

    default:
      "px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 flex items-center gap-1",
  },
};
...
export const bannerStyles = {
  ...
  bannerContainer: "relative w-full max-w-7xl mx-auto my-12 px-4 mt-40",
  ...
  bookButton: "relative overflow-hidden bg-gradient-to-r from-green-500 via-emerald-500 to-green-400 text-white px-7 py-3 sm:px-9 sm:py-4 rounded-full font-semibold tracking-wide transition-all duration-300 shadow-lg hover:shadow-2xl hover:scale-105 active:scale-95",
  bookButtonOverlay: "pointer-events-none absolute inset-0 bg-white/20 transform -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700",
  bookButtonContent: "relative z-10 flex items-center justify-center gap-3",
...
export const certificationStyles = {
 ...
 animationStyles: `
    ...
    .logosFlexContainer:hover .animate-marquee-single {
...
export const contactPageStyles = {
  // Page container - Enhanced gradient and animation
  pageContainer: "min-h-screen mt-15 relative bg-gradient-to-br from-emerald-50 via-white to-teal-50 py-12 px-4 sm:px-6 md:px-8 lg:px-20 font-sans overflow-hidden",

  // Enhanced background accents with multiple layers
  bgAccent1: "absolute top-20 left-10 w-72 h-72 bg-gradient-to-r from-emerald-300 to-teal-300 rounded-full blur-3xl opacity-20 animate-pulse",
  bgAccent2: "absolute bottom-0 right-10 w-96 h-96 bg-gradient-to-r from-emerald-400 to-cyan-400 rounded-full blur-3xl opacity-15 animate-spin-slow",
  bgAccent3: "absolute top-1/2 left-1/3 w-64 h-64 bg-amber-200 rounded-full blur-3xl opacity-10 animate-float",
  bgAccent4: "absolute bottom-20 left-1/4 w-48 h-48 bg-rose-200 rounded-full blur-3xl opacity-10 animate-float-delay",

  // Floating particles
  particle1: "absolute top-10 right-20 w-2 h-2 bg-emerald-400 rounded-full animate-ping-slow",
  particle2: "absolute bottom-40 left-10 w-3 h-3 bg-teal-400 rounded-full animate-bounce-slow",
  particle3: "absolute top-1/3 right-1/4 w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse-slow",

  // Grid and layout
  gridContainer: "max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start relative z-10",

  // Form container - Glass morphism effect
  formContainer: "relative bg-white/80 backdrop-blur-xl shadow-2xl rounded-3xl border border-emerald-100 p-6 sm:p-8 md:p-10 transition-all duration-500 hover:shadow-emerald-200/50",

  // Badge
  badgeContainer: "inline-flex items-center gap-2 bg-emerald-100 px-4 py-1.5 rounded-full mb-6",
  badgeIcon: "w-4 h-4 text-emerald-600",
  badgeText: "text-xs font-semibold text-emerald-700 uppercase tracking-wider",

  // Form title with underline animation
  formTitle: "text-3xl sm:text-4xl font-bold text-emerald-900 mb-2 relative inline-block",
  titleUnderline: "absolute -bottom-2 left-0 w-12 h-1 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full",

  formSubtitle: "text-sm sm:text-base text-emerald-600 mb-8",

  // Form layout
  formGrid: "grid grid-cols-1 sm:grid-cols-2 gap-5",
  formSpace: "space-y-6",

  // Input groups with animations
  inputGroup: "space-y-2",

  // Enhanced labels
  label: "text-emerald-800 text-sm font-semibold flex items-center gap-2",
  labelIcon: "text-emerald-500",

  // Inputs with focus effects
  input: "w-full px-5 py-3 border-2 border-emerald-200 bg-white rounded-2xl focus:outline-none transition-all duration-300 text-sm sm:text-base shadow-sm hover:shadow-md",
  inputFocused: "border-emerald-400 shadow-lg shadow-emerald-100 ring-4 ring-emerald-100",

  textarea: "w-full px-5 py-3 border-2 border-emerald-200 bg-white rounded-2xl focus:outline-none transition-all duration-300 text-sm sm:text-base shadow-sm hover:shadow-md resize-none",
  textareaFocused: "border-emerald-400 shadow-lg shadow-emerald-100 ring-4 ring-emerald-100",

  // Error messages
  error: "text-xs text-rose-500 mt-1 flex items-center gap-1 animate-shake",

  // Enhanced button
  buttonContainer: "flex flex-col items-center gap-3 pt-2",
  button: "group w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-3 rounded-2xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl hover:scale-105 active:scale-95",
  buttonIcon: "transition-transform group-hover:translate-x-1",
  buttonArrow: "transition-transform group-hover:translate-x-1 opacity-0 group-hover:opacity-100",

  sentMessage: "text-emerald-600 text-sm flex items-center gap-2 animate-fade-in",

  // Right side info container
  infoContainer: "space-y-6",

  // Enhanced info card
  infoCard: "relative bg-white rounded-3xl p-6 shadow-xl border border-emerald-100 overflow-hidden",
  cardDecoration: "absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-emerald-100 to-transparent rounded-full -mr-16 -mt-16",

  infoTitle: "text-2xl font-bold text-emerald-800 mb-4",
  infoDivider: "w-12 h-1 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-full mb-5",

  infoItems: "space-y-4 mb-6",
  infoItem: "flex items-start gap-3",
  infoIconWrapper: "w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600",
  infoItemLabel: "text-xs text-emerald-500 font-medium",
  infoItemValue: "text-sm text-gray-700 font-medium",

  emergencyBadge: "mt-4 p-3 bg-rose-50 rounded-xl flex items-center gap-2 text-rose-600 text-sm font-medium border border-rose-200",

  // Stats grid
  statsGrid: "grid grid-cols-2 gap-3",
  statCard: "bg-white rounded-2xl p-4 text-center border border-emerald-100 hover:shadow-lg transition-all duration-300 hover:scale-105",
  statIcon: "w-8 h-8 mx-auto mb-2",
  statValue: "text-xl font-bold text-gray-800",
  statLabel: "text-xs text-gray-500",

  // Map container
  mapContainer: "rounded-3xl overflow-hidden shadow-xl border-2 border-emerald-100 hover:shadow-2xl transition-all duration-300",
  map: "w-full h-56 sm:h-64 md:h-72 object-cover",

  // Hours section
  hoursContainer: "bg-gradient-to-br from-emerald-100 to-teal-100 rounded-2xl p-5 shadow-inner mt-8 border border-emerald-200",
  hoursHeader: "flex items-center gap-2 mb-4",
  hoursIcon: "text-emerald-700",
  hoursTitle: "text-lg font-semibold text-emerald-800",
  hoursContent: "space-y-3",
  hoursRow: "flex justify-between items-center text-sm text-gray-700",
  hoursTime: "font-semibold text-emerald-700",
  hoursNote: "mt-3 pt-3 border-t border-emerald-200 text-xs text-emerald-600 flex items-center gap-1",
...
export const doctorsPageStyles = {
  // Main container
  mainContainer: "min-h-screen mt-20 bg-linear-to-br from-emerald-50 to-teal-100 py-8 sm:py-10 px-3 sm:px-6 relative overflow-hidden",
...
export const footerStyles = {
  ...
  newsletterForm: "w-full max-w-md mx-auto lg:mx-0",

  // Mobile newsletter - button below input
  mobileNewsletterContainer: "flex flex-col gap-3 lg:hidden w-full",
  ...
  // Desktop newsletter - button below input
  desktopNewsletterContainer: "hidden lg:flex lg:flex-col gap-3 w-full",
  desktopEmailInput: "w-full px-6 py-4 text-base text-emerald-800 bg-white border-2 border-emerald-200 rounded-full focus:outline-none focus:ring-4 focus:ring-emerald-300 focus:border-emerald-400 transition-all duration-300 shadow-lg placeholder-emerald-400",
  desktopSubscribeButton: "w-full inline-flex items-center justify-center gap-2 py-3 rounded-full bg-gradient-to-r from-emerald-500 to-green-600 text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-300",
  desktopButtonIcon: "w-4 h-4",
...
export const loginPageStyles = {
  // Main container
  mainContainer: "min-h-screen flex items-stretch relative font-sans overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900",

  // Professional background pattern
  bgPattern: "absolute inset-0 opacity-5 bg-[url('data:image/svg+xml,%3Csvg%20width=%2260%22%20height=%2260%22%20viewBox=%220%200%2060%2060%22%20xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cg%20fill=%22none%22%20fill-rule=%22evenodd%22%3E%3Cg%20fill=%22%23ffffff%22%20fill-opacity=%220.4%22%3E%3Cpath%20d=%22M36%2034v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6%2034v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6%204V0H4v4H0v2h4v4h2V6h4V4H6z%22/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')]",

  // Animated background gradients - Reduced size
  bgGradient: "absolute inset-0 bg-gradient-to-br from-emerald-500/10 via-transparent to-teal-500/10",
  bgBlur1: "absolute top-0 left-0 w-[300px] h-[300px] bg-emerald-400 rounded-full blur-[100px] opacity-20 animate-pulse",
  bgBlur2: "absolute bottom-0 right-0 w-[300px] h-[300px] bg-teal-400 rounded-full blur-[100px] opacity-20 animate-pulse animation-delay-2000",

  // Floating icons - Reduced size
  floatingIcon1: "absolute top-20 left-10 animate-float opacity-10 hidden lg:block",
  floatingIcon2: "absolute bottom-20 right-10 animate-float animation-delay-2000 opacity-10 hidden lg:block",
  floatingIcon3: "absolute top-1/3 right-1/4 animate-float animation-delay-3000 opacity-10 hidden lg:block",
  floatingIcon4: "absolute bottom-1/3 left-1/4 animate-float animation-delay-4000 opacity-10 hidden lg:block",
  floatingIconSvg: "w-12 h-12 text-white",

  // Professional Banner with Image - Reduced padding
  bannerOverlay: "hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-emerald-900 to-teal-900",
  bannerImageContainer: "absolute inset-0",
  bannerImage: "w-full h-full object-cover opacity-80",
  bannerGradient: "absolute inset-0 bg-gradient-to-br from-emerald-900/90 via-emerald-800/80 to-teal-900/90",
  bannerContent: "relative z-10 flex flex-col justify-center h-full p-8 lg:p-10 text-white",

  badge: "inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-full w-fit mb-4 text-sm",
  badgeIcon: "w-3 h-3 text-emerald-300",

  bannerTitle: "text-3xl lg:text-4xl font-bold mb-3 leading-tight",
  titleHighlight: "block text-emerald-300 text-2xl lg:text-3xl mt-1",
  bannerSubtitle: "text-emerald-100 text-base mb-6 leading-relaxed",

  // Stats - Reduced size
  statsContainer: "flex gap-4 mb-6",
  statItem: "flex items-center gap-2",
  statIconWrapper: "w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center",
  statIcon: "w-4 h-4 text-emerald-300",
  statValue: "text-lg font-bold",
  statLabel: "text-xs text-emerald-200",

  // Feature list - Reduced spacing
  featureList: "space-y-2 mb-6",
  featureItem: "flex items-center gap-2 text-emerald-100 text-sm",
  featureIcon: "w-4 h-4 bg-emerald-500 rounded-full flex items-center justify-center text-white text-xs font-bold",

  // Testimonial - Reduced size
  testimonial: "mt-6 p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-white/10",
  testimonialQuote: "text-emerald-100 italic text-sm mb-3",
  testimonialAuthor: "flex items-center gap-2",
  authorAvatar: "w-8 h-8 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full",
  authorName: "text-xs font-semibold",
  authorTitle: "text-xs text-emerald-300",

  // Login Card - Smaller, more rounded
  loginCard: "relative z-20 w-full lg:w-[420px] bg-white shadow-2xl rounded-3xl flex flex-col justify-center p-6 lg:p-8 overflow-y-auto my-8 lg:my-20 lg:ml-auto lg:mr-30",
  // Back button - Smaller
  backButton: "absolute top-4 left-4 cursor-pointer flex items-center gap-1.5 text-gray-500 font-medium hover:text-emerald-600 transition-all duration-300 group text-sm",
  backButtonIcon: "w-3.5 h-3.5 transition-transform group-hover:-translate-x-1",

  // Logo - Smaller
  logoContainer: "flex items-center justify-center gap-2 mb-6",
  logo: "w-10 h-10 object-contain",
  logoText: "flex flex-col",
  logoName: "text-lg font-bold text-gray-800",
  logoTagline: "text-[10px] text-gray-500",

  // Header - Smaller
  title: "text-xl font-bold text-center text-gray-800 mb-1",
  subtitle: "text-center text-gray-500 text-sm mb-6",
  ...
  inputIcon: "absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4",

  // Input fields - Smaller
  input: "w-full pl-9 pr-9 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:outline-none transition-all duration-300 text-gray-700 text-sm",
  inputFocused: "border-emerald-400 ring-2 ring-emerald-100 bg-white",

  // Eye button
  eyeButton: "absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors",

  // Form footer - Smaller
  formFooter: "flex items-center justify-between text-sm",
  rememberMe: "flex items-center gap-1.5 text-sm text-gray-600 cursor-pointer",
  checkbox: "w-3.5 h-3.5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500",
  forgotLink: "text-sm text-emerald-600 hover:text-emerald-700 transition-colors",

  // Submit button - Smaller
  submitButton: "w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-xl transition-all duration-300 hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed group text-sm",
  buttonArrow: "w-4 h-4 transition-transform group-hover:translate-x-1",
  ...
  dividerText: "block text-xs text-gray-400 text-center relative bg-white px-3 w-fit mx-auto",

  // Footer - Smaller
  footer: "mt-4 text-center",
  securityBadge: "flex items-center justify-center gap-1 text-xs text-gray-400 mb-1.5",
...
export const servicePageStyles = {
  // Page container
  pageContainer: "min-h-screen mt-30 py-12 px-6 lg:px-20 font-serif bg-linear-to-b from-emerald-50 to-white",
...
export const testimonialStyles = {
  ...
  // Testimonial card
  testimonialCard: "bg-white font-[pacifico] rounded-xl shadow-lg p-4 sm:p-5 mb-4 transition-all duration-300 border-l-4 w-full max-w-xl mx-auto hover:scale-[1.02]",
...
export const navbarStylesDr = {
  ...
  mobileMenuContainer: (isOpen) =>
    `lg:hidden mt-15 fixed top-20 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-md bg-white/95 backdrop-blur-md border border-emerald-100 rounded-2xl shadow-lg transform origin-top transition-all duration-300 ${isOpen ? "scale-100 opacity-100 translate-y-0" : "scale-95 opacity-0 -translate-y-4 pointer-events-none"}`,
  ...
  // In your styles, add:
  mobileActiveLink: "bg-emerald-50 text-emerald-700 font-semibold",
  mobileInactiveLink: "text-emerald-600 hover:bg-emerald-50",
...
export const listPageStyles = {
  // Main container
  pageContainer: "min-h-screen mt-5 pt-20 md:pt-25 lg:pt-25 font-serif p-4 sm:p-6 bg-linear-to-br from-emerald-50 to-white",
...
export const dashboardStyles = {
  // Main container
  pageContainer: "min-h-screen mt-5 font-serif pt-16 lg:pt-20 md:pt-15 p-4 sm:p-6 bg-linear-to-br from-emerald-50 to-white",
